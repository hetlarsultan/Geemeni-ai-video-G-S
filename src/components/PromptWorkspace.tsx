...
const PromptWorkspace = React.memo(
  ({
    ...
    const [recLipSync, setRecLipSync] = useState(0);
    ...
    const toggleRecording = async () => {
      if (isRecording) {
        ...
        try {
          const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
          setIsRecording(true);
          audioAnalyzer.start(stream, (level) => {
            setRecLipSync(level);
          });
        } catch (err) {
          console.error("Microphone access denied:", err);
          alert("يرجى تفعيل صلاحية الميكروفون لبدء التسجيل الصوتي.");
        }
      }
    };
      ...
      async (customPrompt?: string) => {
        ...
        try {
          ...
          if (mediaType === "character") {
            ...
            if (isOfflineMode) {
              const localResponse = await generateLocalOllama(actualPrompt);
              setGenerations((prev) =>
                prev.map((g) =>
                  g.id === newGen.id
                    ? {
          ...
          <div className="flex justify-between items-center p-4 bg-zinc-900/80 absolute top-0 w-full border-b border-zinc-800/50 backdrop-blur-md z-20">
            ...
            <div className="flex items-center gap-3">
              ...
              <button 
                ...
                className={`flex items-center gap-1.5 px-2 py-0.5 rounded text-[8px] font-bold uppercase transition-all hover:scale-105 active:scale-95 ${
                  isOfflineMode 
                    ? "bg-purple-500/20 text-purple-400 border border-purple-500/40 shadow-[0_0_8px_rgba(168,85,247,0.3)]" 
                    : "bg-zinc-800/40 text-zinc-500 border border-zinc-700/50 hover:text-zinc-300"
                }`}
                title="تفعيل معالجة Ollama المحلية (Llama3)"
              >
                <Cpu className={`w-2.5 h-2.5 ${isOfflineMode ? 'animate-pulse' : ''}`} />
                {isOfflineMode ? "Ollama_Active" : "Local_Sync"}
              </button>
              <div
                    ...
                    config.selectedCharacters.length > 0 ? (
                      <div className="w-full max-w-2xl flex flex-col items-center">
                        <CharacterAnimator 
                          ...
                          lipSyncLevel={isRecording ? recLipSync : lipSyncLevel}
                          config={config}
                        />
                        ...
                        <div className="w-full mt-6 flex justify-between items-center bg-zinc-950/40 p-4 rounded-xl border border-zinc-900">
                           <div className="flex items-center gap-4">
                             <div className="p-3 bg-blue-600/10 rounded-lg border border-blue-500/20">
                               <Mic2 className={`w-5 h-5 ${isRecording ? 'text-rose-500 animate-pulse' : 'text-blue-500'}`} />
                             </div>
                             <div className="space-y-0.5">
                               <div className="text-[9px] font-black text-white uppercase tracking-wider">{isRecording ? 'دخل صوتي مباشر' : 'جسر الصوت العصبي'}</div>
                               <div className="text-[7px] font-mono text-zinc-500 uppercase tracking-tighter">معدل المزامنة: {((isRecording ? recLipSync : lipSyncLevel) * 100).toFixed(1)}%</div>
                             </div>
                           </div>
                            ...
                            <div className="flex gap-1 items-end h-8">
                              {[0.4, 0.9, 0.6, 1, 0.5, 0.8, 0.4].map((h, i) => (
                                <motion.div
                                  ...
                                  animate={{
                                    height: isRecording 
                                      ? [`${Math.max(10, recLipSync * 100 * (0.5 + Math.random()))}%`, `${Math.max(10, recLipSync * 100 * h)}%`] 
                                      : ["20%", `${h * 100}%`, "20%"],
                                  }}
                                  transition={{
                                    duration: isRecording ? 0.05 : 0.15,
                                    repeat: Infinity,
                                    delay: isRecording ? 0 : i * 0.05,
                                  }}
                                  className={`w-1.5 rounded-full ${isRecording ? 'bg-rose-500 shadow-[0_0_8px_rgba(244,63,94,0.5)]' : 'bg-blue-500'}`}
                                />
                              ))}
                            </div>
                            <div className="text-left">
                              <div className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                                <Mic2 className={`w-3 h-3 ${isRecording ? 'text-rose-500 animate-pulse' : 'text-blue-500'}`} />
                                {isRecording ? 'LIVE_MIC_INPUT' : 'Vocal_Sync_Active'}
                              </div>
                              <div className="text-[8px] text-zinc-400 font-mono">
                                {isRecording ? `AMP: ${(recLipSync * 100).toFixed(1)}% | REALTIME: ON` : 'LATENCY: 12ms | ENGINE: V3_PRO_SYNTH'}
                              </div>
                            </div>
                          </div>
                        </div>
            ...
            <div className="flex-grow bento-inner flex flex-col relative group overflow-hidden bg-black/40 min-h-[400px]">
              ...
              <div className="absolute bottom-3 right-3 flex items-center gap-3">
                ...
                <button
                  ...
                  title={isRecording ? "إيقاف التسجيل" : "بدء التسجيل الصوتي المباشر"}
                >
